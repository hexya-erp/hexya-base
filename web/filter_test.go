	...
	Convey("Testing GetFilters logic", t, func() {
		So(models.SimulateInNewEnvironment(security.SuperUserID, func(env models.Environment) {
			...
			Convey("Testing own filters", func() {
				h.Filter().Create(env, &h.FilterData{
					Name:     "a",
					User:     demoUser,
					ResModel: "Filter",
				})
	...
	Convey("Test Own Defaults", t, func() {
		So(models.SimulateInNewEnvironment(security.SuperUserID, func(env models.Environment) {
			...
			Convey("New No Filter", func() {
				h.Filter().NewSet(env).Sudo(demoUser.ID()).CreateOrReplace(&h.FilterData{
					Name:      "a",
					ResModel:  "Filter",
					User:      demoUser,
					IsDefault: true,
	...
	Convey("Test Read Group", t, func() {
		So(models.SimulateInNewEnvironment(security.SuperUserID, func(env models.Environment) {
			Convey("Filter Read Group", func() {
				...
				filterB := h.Filter().Create(env, &h.FilterData{
					Name:     "Filter B",
					ResModel: "Filter",
				})
				...
				res := h.Filter().NewSet(env).SearchAll().
					GroupBy(h.Filter().User()).
					Aggregates(h.Filter().Name(), h.Filter().User())
				var oneFalse bool
				for _, r := range res {
